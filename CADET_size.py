...
def CADET_size(dir, threshold=0.55):
    ...
    for filename in files:
        ...
        ccd.write("{0}/predictions/{1}_CADET_size.fits".format(dir,name), overwrite=True)
        ...
        with open("{0}/txts/{1}_size.txt".format(dir, name), "w") as file:
        ...
        plt.tight_layout()
        plt.savefig("{0}/decomposed/{1}_CADET_size.png".format(dir, name))
        plt.close()
...
if __name__ == "__main__":
    if len(sys.argv) > 2:
        CADET_size(sys.argv[1], float(sys.argv[2]))
    elif len(sys.argv) == 2:
        CADET_size(sys.argv[1])
    else:
        print("\nUsage: python3 CADET_size.py foldername [threshold]")